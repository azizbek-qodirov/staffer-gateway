...
func (h *Handler) CreateNotification(ctx *gin.Context) {
	...
	res, err := h.NotificationStorage.Create(ctx, req)
	if err != nil {
		ctx.JSON(401, gin.H{"error": err.Error()})
		return
	}
...
func (h *Handler) GetNotificationsByUser(ctx *gin.Context) {
	req := &pb.GetAllRequest{
		...
		NotifiationId:  ctx.Query("notification_id"),
	}
	res, err := h.NotificationStorage.GetByUserId(ctx, req)
	if err != nil {
		ctx.JSON(401, gin.H{"error": err.Error()})
		return
	}
...
func (h *Handler) ReadAllNotifications(ctx *gin.Context) {
	...
	res, err := h.NotificationStorage.ReadeAll(ctx, req)
	if err != nil {
		ctx.JSON(401, gin.H{"error": err.Error()})
		return
	}
...
func (h *Handler) SendNotificationToAll(ctx *gin.Context) {
	...
	res, err := h.NotificationStorage.SendAll(ctx, req)
	if err != nil {
		ctx.JSON(401, gin.H{"error": err.Error()})
		return
	}
